;; [[id:fb897e1e-7a91-4e9f-875e-0231fefc420b][10.scm]]
;;
;; Add Application service to [[ref:a3c28cb6-6f1f-4fcb-aea2-15561d9c96b1][9.scm]]
...
;;
;; ApplicationConf#mk : Name Port HostName RootPubKey → ApplicationConf
...
Return the list of Shepherd services for the application."
  ...
  (let* ((name     (application-conf-name conf))
         ...
         (env #~(list (string-append "PHX_SERVER=true")
                      (string-append "PHX_HOST=" #$host)
                      (string-append "PORT=" #$port)
                      (string-append "LC_ALL=" #$locale)
                      (string-append "SECRET_KEY_BASE="
                                     (or (getenv "SECRET_KEY_BASE")
                                         "Z8DeDEXcl3tVcyN6HfEUu60yuMUaKKX29ViWUSF0/PLgPM8DkenrHlJRBahDxUJc"))
                      (string-append "XAI_API_KEY="
                                     (or (getenv "XAI_API_KEY") "sk-0000"))
     ...
     (shepherd-service
      ...
      (start
       #~(make-forkexec-constructor
          (list "bash" "-c" "createdb -U postgres application || true && migrate")
          #:environment-variables #$env))
      (stop #~(make-kill-destructor)))

     ;; Main application server (foreground)
     (shepherd-service
