;; [[ref:631ab476-0770-4049-94df-32e7c3197a0c][Specification]]
;; [[id:71dd6e62-8b3a-433c-917f-518b9f9e1e77]]
...
;; ───────────────
;;
;; Represents configurations values from which the os is derived.
...
(define-record-type* <application-conf>
  ...
  (port application-conf-port (default "4000")) ; TODO(be35): needed? 
  ...
  (cert-pub application-conf-cert-pub (default "/etc/letsencrypt/live/application.com/fullchain.pem"))
  (cert-prv application-conf-cert-prv (default "/etc/letsencrypt/live/application.com/privkey.pem"))
  (store-key application-conf-store-key (default "/etc/guix/signing-key.pub")))
...
;; Environment → ApplicationConf
;; ─────────────────────────────
;;
;; Given the current environment; then: build a matching ApplicationConf.

;; Reads environment variables as defined in [[ref:631ab476-0770-4049-94df-32e7c3197a0c][Specification]].
(define (env->application-conf)
  (application-conf
    (name (getenv "APP_NAME"))
    (version (getenv "APP_VERSION"))
    (dist (getenv "APP_DIST"))
    (host-name (getenv "APP_HOST_NAME"))
    (ssh-root-pub-key-path (getenv "APP_HOST_ROOT_KEY"))
    (cert-pub (getenv "APP_HOST_CERT_PUB"))
    (cert-prv (getenv "APP_HOST_CERT_PRV"))
    (store-key (getenv "APP_STORE_KEY"))))
...
and copies the development TLS certificate and private key with correct permissions"
  ...
  (let* ((certs (application-conf-certs conf))
          (host-cert (local-file (application-conf-cert-pub conf)))
          (cert (application-conf-cert conf))
          (host-cert-key (local-file (application-conf-cert-prv conf)))
    ...
    (shepherd-service
      ...
      (start
        #~(lambda _
            ...
            ;; References:
            ;;   [[ref:15f603ef-6046-448d-a203-c8712313b8d1][/etc is root owned]]
            (for-each
              (lambda (dir)
                (chmod dir #o755))
              '#$root-dirs)

            ;; If dev-mode, then simulate secrets and SSL certificates
            (when #$dev-mode
              (let ((write-secret (lambda (filename value)
                                    (let ((path (string-append #$secret "/" filename)))
                                      (call-with-output-file path
                                        (lambda (port) (display value port)))
                                      (chmod path #o600)))))
                (write-secret "SECRET_KEY_BASE"
                  "Z8DeDEXcl3tVcyN6HfEUu60yuMUaKKX29ViWUSF0/PLgPM8DkenrHlJRBahDxUJc")
                (write-secret "XAI_API_KEY"
                  "sk-000000"))
...
(define (application-init conf)
  ...
  (let* ((name    (application-conf-name conf))
          ...
          (port    (application-conf-port conf))
          (host    (application-conf-host-name conf))
          (locale  (application-conf-locale conf))
          (pkg     (application-conf-package conf))
          (secret  (application-conf-secret conf)))
...
Return the list of Shepherd services for the application."
  (application-conf-check conf)
  (let* ((name     (application-conf-name conf))
          (port     (application-conf-port conf))
          (host     (application-conf-host-name conf))
          (pid-file (application-conf-pid-file conf))
          (log-file (application-conf-log-file conf))
          (pkg      (application-conf-package conf))
          (server   (application-conf-server conf))
          (secret   (application-conf-secret conf))
          (locale   (application-conf-locale conf))
          (env (application-conf-env conf))
          (modules '((shepherd service)
                      (guix build utils)
                      (ice-9 ftw)
                      (ice-9 textual-ports))))
...
(define configuration (env->application-conf))
    ...
    (cons*
      ...
      (modify-services %base-services
                              (guix-service-type
                               ...
                               (guix-configuration
                                (inherit config)
                                 (channels (load (getenv "CHANNELS")))
                                (authorized-keys
                                 (cons (local-file (application-conf-store-key configuration))
