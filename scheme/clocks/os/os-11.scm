;; [[id:67895304-36b7-4cc6-81eb-7292e58c6ae0][11.scm]]
;;
;; Add Application service to [[ref:fb897e1e-7a91-4e9f-875e-0231fefc420b][10.scm]]
...
;; ───────────
;;
;; After execution, the file system contains all necessary files for the application
;; to be executed according to the [[ref:109bc638-0220-4f10-8b08-82d995bc1752][Application and FHS]].
...
(define (application-fhs config)
  ...
  (let* ((name (application-conf-name config))
         ...
         (secret (application-conf-secret config))
         (run (application-conf-run config))
         (log (application-conf-log config))
         (dirs (list etc
                     secret
                     home
                     run
...
Return a G-expression that, when evaluated on the target VPS by the Shepherd
daemon, produces the full list of environment variables."
  ...
  (let* ((name   (application-conf-name conf))
    ...
    #~(list
       ...
       (string-append "PATH=" #$(file-append bash "/bin")
...
(define (application-secrets conf)
  (application-conf-check conf)
  (let ((secret (application-conf-secret conf))
        (dev-mode (application-conf-dev? conf)))
    (shepherd-service
     (provision '(application-secrets))
     (requirement '())
     (one-shot? #t)
     (documentation "Write development secret files")
     (modules '((shepherd service) (guix build utils)))
     (start
      #~(lambda _
          (when #$dev-mode
            (mkdir-p #$secret)
            ...
            (let ((write-secret (lambda (filename value)
              ...
              (write-secret "XAI_API_KEY" "sk-000000")))
          #t))
...
(define (application-init conf)
  ...
  (let ((name (application-conf-name conf))
    ...
    (shepherd-service
     ...
     (requirement '(postgres application-secrets))
     (one-shot? #t)
...
(define (application-master conf)
      ...
      (start
       #~(make-forkexec-constructor
          ...
          (append
           ...
           (map (lambda (filename)
                  (let* ((path  (string-append #$secret "/" filename))
                         (value (string-trim-right (call-with-input-file path get-string-all))))
